...
当前 MVP 的 MySQL 和 Redis 依赖还没有真实接入代码，运行健康检查和代理转发时不需要先启动 MySQL/Redis。
...
`/gateway/*` 会转发到 `.env` 中的 `GATEWAY_UPSTREAM_BASE_URL`。仓库已提供一个本地上游示例目录 `runtime/upstream-demo`，另开一个终端执行：
...
```bash
curl http://127.0.0.1:9000/hello.json
```
...
- 代理示例：`http://127.0.0.1:8000/gateway/hello.json`
- MVP 前端控制台：`http://127.0.0.1:8000/dashboard.html`

### 5. 手工验证接口

```bash
curl http://127.0.0.1:8000/health
curl http://127.0.0.1:8000/gateway/hello.json
curl -i -X OPTIONS http://127.0.0.1:8000/gateway/hello.json -H "Origin: http://localhost:5173"
curl http://127.0.0.1:8000/not-exists
```

预期结果：

- `/health` 返回 `code=SUCCESS`，并包含 `trace_id`。
- `/gateway/hello.json` 返回本地测试上游的响应内容。
- `OPTIONS /gateway/hello.json` 返回 CORS 相关响应头。
...
该命令会自动启动本地测试上游和 ThinkPHP 网关，并验证健康检查、代理转发、CORS、404 兜底和前端控制台页面。

如果默认端口被占用，可以临时指定端口：

```bash
APP_PORT=8010 UPSTREAM_PORT=9010 composer test:smoke
```
